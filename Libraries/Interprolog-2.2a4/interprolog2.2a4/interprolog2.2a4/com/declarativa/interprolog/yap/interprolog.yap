/*** YAP Prolog-dependent stuff; for copyright information read top of com/declarativa/interprolog/xsb/interprolog.P ***/

:- use_module(library(charsio)), use_module(library(lists)), use_module(library(system)).

% socket_get0(Sockfd,C) :- get0(Sockfd,C).
% socket_get0(Sockfd,C) :- get_byte(Sockfd,C).
socket_get0(Sockfd,C) :- get_byte(Sockfd,_C), (_C<0 -> C is 256+_C ; C=_C).
% socket_get0(Sockfd,C) :- get_code(Sockfd,_C), (_C<0 -> C is 256+_C ; C=_C).
/* YAP is currently (5.0, Windows) not transparent and gets stuck when getting -1, so this is a 
turnaround; the extra escape chars don't count for readNSocketChars */

%socket_get0(Sockfd,C) :- get_byte(Sockfd,_C), socket_get0(_C,Sockfd,C).

%socket_get0(101 /* Escape char, see PrologOutputObjectStream.java */,Sockfd,C) :- !, 
%	get_byte(Sockfd,_C), 
%	(_C=1 -> C=101; (_C=2->C=255; ipProgressMessage('bad char following escape'-_C),fail)).
%socket_get0(_C,_,C) :-
%	(_C<0 -> C is 256+_C ; C=_C).
%*/
% socket_put(Sockfd,C)  :- put(Sockfd,C).
socket_put(Sockfd,C)  :- put_byte(Sockfd,C).
%socket_put(Sockfd,C)  :- (C >127 -> _C is C-256 ; _C=C), put_byte(Sockfd,_C).

writeAllSocketChars(More,S) :- writeAllSocketChars_(More,S), flush_output(S).

ipinitsockets(Host,Sport) :-
	ipProgressMessage('Creating socket'-socket('AF_INET',S)),
	%  socket('AF_INET',S), 
	socket('AF_INET','SOCK_STREAM',0,S), 
	ipProgressMessage('Created socket'-S),
	(socket_connect(S, 'AF_INET'(Host, Sport), Stream)->true;
		write('Weird socket connect failure 1'),nl,fail),
	ipProgressMessage('Connected socket'-S),
	% stream_property(Stream,type(binary)), HOW TO CREATE A BINARY STREAM?
	asserta(ipsocketstreams(Stream, Stream)),
	ipLearnExamples, 
...
setupWindowsInterrupt(Host,INTport) :-
	...
	%socket('AF_INET',IntS),
	socket('AF_INET','SOCK_STREAM',0,IntS), 
	ipProgressMessage('Created interrupt socket'-IntS),
	(socket_connect(IntS, 'AF_INET'(Host, INTport), Stream)->true;
		write('Weird socket connect failure 3'),nl,fail),
	setupJavaInterrupt(Stream).


% This is specific for UNIX, does not apply to Windows 95/NT
getPrologPID(N) :- pid(N).

%adds a dot at the end
atomRead(Atom,Term) :- catch((atom_codes(Atom, Chars), read_from_chars(Chars, Term)),_E,Term=end_of_file).



reset_dcg_mode. % do nothing - to avoid warning
...
% turnaround hack, YAP 5.0.0 doesn't bind variable properly in throw/catch; hopefully not for long...if not, should use goal timestamps here
% get_ip_exception_description(E,interprolog_interrupt) :- var(E), retract(ipYAPthrowHack), !. 
get_ip_exception_description(error(type_error(callable,_),_), interprolog_interrupt).
get_ip_exception_description(E, E).

% :-dynamic ipYAPthrowHack/0.
...
double(X) --> [B7,B6,B5,B4,B3,B2,B1,B0], 
	{var(X) -> analyseDouble(B7,B6,B5,B4,B3,B2,B1,B0,S,M1,M2,E), cvtDouble(S,E,M1,M2,X)
			 ; float(X),
			 (X=0.0 -> B7=0, B6=0, B5=0, B4=0, B3=0, B2=0, B1=0, B0=0 ;
			  cvtDouble(S,E,M1,M2,X), generateDouble(B7,B6,B5,B4,B3,B2,B1,B0,S,M1,M2,E)) }.
...
% These two contributed by Tatyana Vidrevich:
...
float_to_intrep(Sign,Power,Mantissa,Float) :-
    ...
    % "This last seems needed for rounding."..? I don't understand why.. hope not. MC
