/*** GNU Prolog-dependent stuff; for copyright information read top of com/declarativa/interprolog/xsb/interprolog.P ***/
...
ipinitsockets(Host,Sport) :-
    working_directory(D), write(directory-D), nl,
	ipProgressMessage('Creating socket'-socket('AF_INET',S)),
	socket('AF_INET',S), 
	ipProgressMessage('Created socket'-S),
	(socket_connect(S, 'AF_INET'(Host, Sport), InStream, OutStream)->true;
...
setupWindowsInterrupt(Host,INTport) :-
	...
	socket('AF_INET',IntS),
	(socket_connect(IntS, 'AF_INET'(Host, INTport), InStream, _OutStream)->true;
...
getPrologPID(N) :- prolog_pid(N).

atomRead(Atom,Term) :- catch(read_from_atom(Atom,Term),_E,Term=end_of_file).

reset_dcg_mode. % do nothing - to avoid warning

setupJavaInterrupt(_InStream) :- true. % needs to call built-in, cf. inst.h. in XSB Prolog
...
handle_ip_exception(Exception, ExceptionModel):-
	...
	buildTermModel(ExceptionDescription, ExceptionModel).
...
handleDeterministicGoal(Goal,RVars,Error,Timestamp,NewBytes)  :- 
	...
	(	specifyPrologResult(Timestamp,Succeeded,NewRVars,Result,Error2), ipProgressMessage(aboutToCall-streamContents([Result],_,NewBytes,[])), streamContents([Result],_,NewBytes,[]) -> 
		Error2=Error
		;
