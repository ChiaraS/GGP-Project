...
public class GNUPeer extends PrologImplementationPeer{
	public GNUPeer(AbstractPrologEngine engine){
		super(engine);
		operators = new PrologOperatorsContext(PrologOperatorsContext.standardGNUOperators);
		systemName = "GNU Prolog";
	}
	public String getBinDirectoryProperty(Properties p){
		return p.getProperty("GNU_BIN_DIRECTORY");
	}
	public String executablePath(String d){
		return d + File.separator + "gprolog";
	}
	public String fetchPrologNumericVersion(){
		Object[] bindings = engine.deterministicGoal("current_prolog_flag(prolog_version, Version)",
			"[string(Version)]");
	...
	public boolean isInterrupt(Object error){
		return false; // TODO! error.toString().startsWith("?????");
	}
}