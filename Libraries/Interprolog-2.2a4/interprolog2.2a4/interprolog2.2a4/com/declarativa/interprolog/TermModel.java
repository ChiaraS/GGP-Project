...
public class TermModel implements Serializable,TreeModel{
	...
	/** This should be true only if this is a list term represented in the children array, no binary (TermModel) tree needed.
	Only closed (non var tail) lists can have the flat representation*/
	protected boolean isFlatList;
	...
	public static ObjectExamplePair example(){
		return new ObjectExamplePair("TermModel",
			new TermModel(new Integer(1),(TermModel[])null,false,false),
			new TermModel(".",new TermModel[2],true,true)
			);
	}
	...
	public TermModel(Object n,TermModel[] c,boolean isAList){
		this(n,c,isAList,false);
	}
	
	public TermModel(Object n,TermModel[] c,boolean isAList, boolean isFlatList){
		children=c;
		node=n; 
		hasListFunctor = isAList;
		this.isFlatList=isFlatList;
                
		if ((hasListFunctor||isFlatList) && !node.equals(".") && !node.equals("[]")) 
	...
	public String toString(PrologOperatorsContext ops, boolean quoted){
		if (getChildCount()==0) {
			if (isAtom() && Character.isUpperCase(node.toString().charAt(0))) return "'"+node+"'";
			else return node.toString();
		} else if (isFlatList) {
                    return flatListToString(ops,quoted);
                }
		else if (isList()) {
                    return listToString(ops,quoted);
                }
		else if (children.length==1){
			if (ops.prefixOperator(node)&&!quoted)
				return node+" "+children[0].toString(ops,quoted);
			else if (ops.postfixOperator(node)&&!quoted)
				return children[0].toString(ops,quoted)+" "+node;
			else
				return node+"("+children[0].toString(ops,quoted)+")";
		} else if (children.length==2 && ops.infixOperator(node) &&!quoted) {
			return children[0].toString(ops,quoted)+node+children[1].toString(ops,quoted);
		} else { // children.lenght>=2
			StringBuffer s= new StringBuffer(node.toString()+"("+children[0].toString(ops,quoted));
	...
	public String listToString(PrologOperatorsContext ops,boolean quoted){
		...
		TermModel temp = this;
		for( i = 0 ; i < (!quoted?listMaxLength:getChildCount()) ; i++ ){
	...
	public static TermModel[] flatList(TermModel list){
		if (list.isFlatList) return list.children;
