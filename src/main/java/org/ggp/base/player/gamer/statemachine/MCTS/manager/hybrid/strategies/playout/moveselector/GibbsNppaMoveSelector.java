...
public class GibbsNppaMoveSelector extends NppaMoveSelector {
	 ...
	 */
	private double nppaTemperature;

	public GibbsNppaMoveSelector(GameDependentParameters gameDependentParameters, Random random,
			GamerSettings gamerSettings, SharedReferencesCollector sharedReferencesCollector) {
		super(gameDependentParameters, random, gamerSettings, sharedReferencesCollector);

		this.nppaTemperature = gamerSettings.getDoublePropertyValue("MoveSelector.nppaTemperature");
	...
	private List<Pair<Integer,Double>> getPlayoutProbabilities(int roleIndex, List<Move> roleMoves){
		...
		for(int j = 0; j < roleMoves.size(); j++){
			legalMovesForWinnerExponential[j] = Math.exp(this.nppaTemperature * this.computeNppaValue(roleIndex, roleMoves.get(j)));
	...
	public String getComponentParameters(String indentation) {
		...
		if(params != null){
			return params + indentation + "NPPA_TEMPERATURE = " + this.nppaTemperature;
		}else{
			return indentation + "NPPA_TEMPERATURE = " + this.nppaTemperature;
		}
