...
public class PnMaximumScoreChoice implements PnMoveChoiceStrategy {
	...
	public PnCompleteMoveStats chooseBestMove(MctsNode initialNode) {
		...
		if(toReturn instanceof PnDecoupledMCTSMoveStats){
			return (PnDecoupledMCTSMoveStats)toReturn;
		}else if(toReturn instanceof PnSequentialMCTSMoveStats){
			return new PnCompleteMoveStats(toReturn.getVisits(), toReturn.getScoreSum(), ((PnSequentialMCTSNode)initialNode).getAllLegalMoves().get(this.myRole.getIndex()).get(bestMoveIndex));
		}else if(toReturn instanceof PnSlowSequentialMCTSMoveStats){
			return (PnSlowSequentialMCTSMoveStats)toReturn;
		}else{
			throw new RuntimeException("MaximumScoreChoice-chooseBestMove(): detected a node of a non-recognizable sub-type of class InternalPropnetMCTreeNode.");
		}
	}
