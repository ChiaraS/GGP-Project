...
public class EpsilonNppaMoveSelector extends NppaMoveSelector {

	/**
	 * With probability nppaEpsilon select a random action, otherwise select the action with
	 * highest weight
	 */
	private double nppaEpsilon;

	public EpsilonNppaMoveSelector(GameDependentParameters gameDependentParameters, Random random,
			GamerSettings gamerSettings, SharedReferencesCollector sharedReferencesCollector) {
		super(gameDependentParameters, random, gamerSettings, sharedReferencesCollector);

		this.nppaEpsilon = gamerSettings.getDoublePropertyValue("MoveSelector.nppaEpsilon");
	...
	public Move getMoveForRole(MctsNode node, MachineState state, int roleIndex)
		...
		if(this.random.nextDouble() < this.nppaEpsilon){
	...
	private Move getBestMove(int roleIndex, List<Move> moves) {
		...
		for(int moveIndex = 0; moveIndex < moves.size(); moveIndex++){
			...
			currentValue = this.computeNppaValue(roleIndex, move);
	...
	public String getComponentParameters(String indentation) {
		...
		if(params != null){
			return params + indentation + "NPPA_EPSILON = " + this.nppaEpsilon;
		}else{
			return indentation + "NPPA_EPSILON = " + this.nppaEpsilon;
		}
