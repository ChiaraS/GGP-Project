...
public class PNStatsAggragator {
		...
		for(File currentFile : theSubFiles){
			if(currentFile.isDirectory()){
				...
				if(!theDestinationFile.exists()){
					...
					columnsFormat = new COL_TYPE[theSplitHeader.length];
					...
					for(int i = 0; i < theSplitHeader.length; i++){
						...
						if(theSplitHeader[i].equals("MCSIterationsPerSecond") ||
							...
							columnsFormat[i] = COL_TYPE.DOUBLE;
						}else{
							columnsFormat[i] = COL_TYPE.LONG;
						}
	...
	private static boolean summarizeSingleFile(String gameKey, File theCSVFile, String theHeader, COL_TYPE[] columnsFormat, String destinationFileName, SingleValueDoubleStats[] statsOfAverages, SingleValueDoubleStats[] statsOfConfidenceIntervals){
