 ...
 *
 */
public class CShufflingIterativeDeepeningGamer extends SampleGamer {

	/**
	 *
	 */
	public CShufflingIterativeDeepeningGamer() {
	...
	public ExplicitMove stateMachineSelectMove(long timeout)
		...
		if(moves.size() != 1){
			...
			MinMax search = new MinMax(true, "C:\\Users\\c.sironi\\BITBUCKET REPOS\\GGP-Base\\LOG\\ShufflingIterativeDeepeningLog.txt", stateMachine.getActualStateMachine());

			Move currentBestMove;

			for(int i = 1; i <= Integer.MAX_VALUE; i++){

				currentBestMove = search.bestmove(finishBy, stateMachine.convertToExplicitMachineState(getCurrentState()), stateMachine.convertToExplicitRole(getRole()), true, 0, 100, i, true, false, new EvalZero(stateMachine.getActualStateMachine()));
