	...
	public ExplicitMove stateMachineSelectMove(long timeout)
		...
		if(moves.size() != 1){
			...
			for(int i = 1; i <= Integer.MAX_VALUE; i++){
				...
				currentBestMove = search.bestmove(finishBy, stateMachine.convertToExplicitMachineState(getCurrentState()), stateMachine.convertToExplicitRole(getRole()), true, 0, 100, i, false, false, new EvalProximity(stateMachine.getActualStateMachine()));
