...
public class CProximityGamer extends SampleGamer {
	...
	public ExplicitMove stateMachineSelectMove(long timeout)
		...
		if(moves.size() != 1){
			...
			MinMax search = new MinMax(true, "C:\\Users\\c.sironi\\BITBUCKET REPOS\\GGP-Base\\LOG\\ProximityLog.txt", stateMachine.getActualStateMachine());
			selection = search.bestmove(finishBy, stateMachine.convertToExplicitMachineState(getCurrentState()), stateMachine.convertToExplicitRole(getRole()), true, 0, 100, 12, false, false, new EvalProximity(stateMachine.getActualStateMachine()));
		}
