...
public class CAlphaBetaGamer extends SampleGamer {
	...
	public ExplicitMove stateMachineSelectMove(long timeout)
		...
		if(moves.size() != 1){
			...
			MinMax search = new MinMax(true, "C:\\Users\\c.sironi\\BITBUCKET REPOS\\GGP-Base\\LOG\\AlphaBetaLog.txt", stateMachine.getActualStateMachine());
			selection = search.bestmove(finishBy, stateMachine.convertToExplicitMachineState(getCurrentState()), stateMachine.convertToExplicitRole(getRole()), true, 0, 100, Integer.MAX_VALUE, false, false, new EvalZero(stateMachine.getActualStateMachine()));
		}
