 ...
 *
 */
public class DynamicPropnetStructureChecker {
	...
	public static void main(String[] args){
		...
		if(args.length == 0){
			...
			GamerLogger.setSpilloverLogfile(game + "DynamicPropnetStructureCheckerTable.csv");
		    GamerLogger.log(FORMAT.CSV_FORMAT, game + "DynamicPropnetStructureCheckerTable.csv", "Game key;Construction Time (ms);Init Time (ms);Check Duration (ms);Pass;");

		    if(game.equals("ALL")){
		    	GameRepository theRepository = GameRepository.getDefaultRepository();
		        for(String gameKey : theRepository.getGameKeys()) {
		            if(gameKey.contains("laikLee")) continue;
		            GamerLogger.log(FORMAT.CSV_FORMAT, game + "DynamicPropnetStructureCheckerTable.csv", checkPropnetStructure(gameKey, maxPropnetConstructionTime));
		            System.out.println();
		        }
				...
				if(theGame == null){
					...
					GamerLogger.log(FORMAT.CSV_FORMAT, game + "DynamicPropnetStructureCheckerTable.csv", checkPropnetStructure(game, maxPropnetConstructionTime));
				}
			}
		}
	}
	...
	public static String checkPropnetStructure(String gameKey, long maxPropnetCreationTime){
		...
		SeparateInternalPropnetManager creator = new SeparateInternalPropnetManager(description, maxPropnetCreationTime);

		Match fakeMatch = new Match(gameKey + "." + System.currentTimeMillis(), -1, -1, -1,theRepository.getGame(gameKey) );

        GamerLogger.startFileLogging(fakeMatch, "DynamicPropnetStructureChecker");
    	...
    	try{
        	...
        	propNet = creator.getDynamicPropnet();
    	...
    	if(propNet == null){
    		...
    		return gameKey + ";" + creator.getPropnetConstructionTime() + ";" + creator.getTotalInitTime() + ";-1;FALSE;";
    	}else{
    		/*
			...
			*/
    		...
    		boolean pass = DynamicPropNetFactory.checkPropnetStructure(propNet);
    		...
    		return gameKey + ";" + creator.getPropnetConstructionTime() + ";" + creator.getTotalInitTime() + ";" + checkDuration + ";" + pass + ";";
    	}
	}
